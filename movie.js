...
movie=[
	...
	},
	{
     ...
     function render_html(array_to_render){
      array_to_render.forEach(function(element, index) {
        
        var anchor=$("<a/>");
        $(anchor).addClass('inline-block');
        $(anchor).prop('href','moviedetail.html?id='+element.id);
        var div = $("<div></div>");
        $(div).addClass('main');
        $(anchor).append(div);
        $(".themain").append(anchor);
       ...
       img.addClass('pic');
        ...
        var title_container = $("<div></div>");
        $(title_container).addClass('title');
        var name=$('<h5>',
        {
        	text:element.name,
        });
        name.addClass('heading');
        name.appendTo(title_container);
        $(div).append(title_container);
        var heading_container = $("<div></div>");
        $(heading_container).addClass('genure');
        var gen=$('<h5>',
        {
        	text:element.genres,
        });
        gen.addClass('heading');
        gen.appendTo(heading_container);
        $(div).append(heading_container);
        });
    ...
    function getid(){
    	var movie_id=urlParam('id');
        var movie_obj = $.map(movie, function(obj){if(obj.id==movie_id)
        	{return obj;}
        })
        $('.themain').html('');
        movie_obj = movie_obj[0];
     
        $("#movie_poster").attr("src", movie_obj.img);
        $("#movie_title").html(movie_obj.name);
        $("#movie_description").html(movie_obj.description);
        $("#movie_genres").html(movie_obj.genres.join(", "));
        $("#movie_rate").html(movie_obj.rate);
        $("#movie_length").html(movie_obj.length);
    }
    ...
    function search(){
    	var search_value=$('#search_me').val().toLowerCase();
    	var movie_obj = $.map(movie, function(obj){
        var search_str = obj.name.substr(0, search_value.length).toLowerCase();
    		console.log(search_str);
    		if(search_str==search_value){
    			return obj;
    		}
        })
        $(".themain").html("");
        render_html(movie_obj);
